# This file contains same code as att model except mean line change which didn't perform well
...
#                 pdb.set_trace()
#                 it = seq[:, :i+1].clone()            #it = seq[:, i].clone()
            else:
                it = seq[:, :i+1].clone()
#                 if i>0:
#                     it = it.sum(1, keepdim=True) // i #added by AK
...
#         if self.training == True:
        if len(xt.shape) > 2:
            xt = xt.mean(1, keepdim = True).squeeze(1)
...
#         pdb.set_trace()
        prev_h = state[0][-1]
#         if fc_feats.shape != xt.shape:
#             print(xt.shape, fc_feats.shape)
