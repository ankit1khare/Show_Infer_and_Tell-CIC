...
#             self.coco_annotation = self.coco_json#json.load(open(self.coco_json))
            ...
            #Getting all the files sequentially
            n = 1
...
#         att_batch = np.ndarray((batch_size, 2048), dtype = 'float32')
        fc_batch = []
        att_batch = []
        max_index = self.N
        wrapped = False
...
#             pdb.set_trace()
            img = self.ids[ri] 
            info_struct = {}
            info_struct['id'] = self.coco_json['images'][ri_next]['id']
#             info_struct['file_path'] = self.coco_json['images'][ri_next]['file_path']
